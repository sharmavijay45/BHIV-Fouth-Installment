...
from agent_bucket import get_vaani_integration, enhanced_agent_response, should_use_vaani
import time
import requests
import json
...
# Vaani Integration Models
class VaaniContentRequest(BaseModel):
    text: str
    content_type: str = "tweet"
    language: str = "en"
    metadata: Optional[Dict[str, Any]] = None

class VaaniTranslationRequest(BaseModel):
    content_id: str
    target_languages: list[str]
    tone: str = "casual"

class VaaniPlatformRequest(BaseModel):
    content_id: str
    platforms: list[str]
    tone: str = "casual"
    language: str = "en"
...
class VaaniEnhancedResponse(BaseModel):
    agent_id: str
    input_text: str
    base_response: str
...
class SimpleResponse(BaseModel):
    ...
    vaani_enhancements: Dict[str, Any] = {} # Added Vaani enhancements
    vaani_used: bool = False # Added Vaani used flag
...
async def process_vedas_query(query: str, user_id: str):
    try:
        sources = engine.search_documents(query, "spiritual")
        context = "\n".join([doc["text"] for doc in sources[:2]])
        prompt = f"""You are a wise spiritual guide. Provide profound insights on: "{query}"

Spiritual context:
{context}

Share wisdom that:
- Connects to ancient spiritual teachings
- Offers practical guidance for modern life
- Inspires inner reflection and growth
- Is respectful of diverse spiritual paths"""
        fallback = f"Thank you for asking about '{query}'. This is a profound spiritual question that touches the heart of human existence. The ancient wisdom traditions teach us that true understanding comes from within, through meditation, reflection, and living with awareness. Consider how this question relates to your own spiritual journey and what insights arise when you sit quietly with it."
        response_text, status = engine.generate_response(prompt, fallback, "vedas")
        
        # Enhance response with Vaani if appropriate
        enhanced_response = enhanced_agent_response("vedas_agent", query, response_text)
        
        return SimpleResponse(
            query_id=str(uuid.uuid4()),
            query=query,
            response=enhanced_response.get("original_response", response_text),
            sources=sources,
            timestamp=datetime.now().isoformat(),
            endpoint="ask-vedas",
            status=status,
            vaani_enhancements=enhanced_response.get("enhanced_features", {}),
            vaani_used=enhanced_response.get("vaani_used", False)
        )
    except Exception as e:
...
- Is suitable for students"""
        ...
        # Enhance response with Vaani if appropriate
        enhanced_response = enhanced_agent_response("edumentor_agent", query, response_text)
...
- Is encouraging and positive"""
        ...
        # Enhance response with Vaani if appropriate
        enhanced_response = enhanced_agent_response("wellness_agent", query, response_text)
...
@app.post("/vaani/content/create")
async def create_vaani_content(request: VaaniContentRequest):
    """Create content using Vaani's content generation model."""
    try:
        # Use default Vaani instance
        vaani = get_vaani_integration()
        if not vaani.access_token:
            if not vaani.authenticate():
                raise HTTPException(status_code=500, detail="Failed to authenticate with Vaani")
        
        result = vaani.create_content(
            text=request.text,
        ...
        logger.error(f"Error creating Vaani content: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/vaani/content/translate")
async def translate_vaani_content(request: VaaniTranslationRequest):
    """Translate content using Vaani's translation model."""
    try:
        # Use default Vaani instance
        vaani = get_vaani_integration()
        if not vaani.access_token:
        ...
        result = vaani.translate_content(
        ...
        logger.error(f"Error translating Vaani content: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/vaani/content/platform")
async def generate_platform_content(request: VaaniPlatformRequest):
    ...
    try:
        ...
        result = vaani.generate_platform_content(
            content_id=request.content_id,
        ...
        if result:
            ...
            raise HTTPException(status_code=500, detail="Failed to generate platform content")
            
    except Exception as e:
        logger.error(f"Error generating platform content: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/vaani/voice/generate")
async def generate_vaani_voice(request: VaaniVoiceRequest):
    ...
    try:
        ...
        result = vaani.generate_voice_content(
            content_id=request.content_id,
            language=request.language,
            tone=request.tone,
            voice_tag=request.voice_tag
        )
        ...
        if result:
            ...
            raise HTTPException(status_code=500, detail="Failed to generate voice content")
            
    except Exception as e:
        logger.error(f"Error generating Vaani voice: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/vaani/enhance-agent-response")
async def enhance_agent_response(agent_id: str, input_text: str, base_response: str):
    """Enhance an agent's response using Vaani capabilities."""
    try:
        result = enhanced_agent_response(agent_id, input_text, base_response)
        return result
    except Exception as e:
        logger.error(f"Error enhancing agent response: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/vaani/health")
async def vaani_health_check():
    """Check Vaani API health status."""
    try:
        # Use default Vaani instance
        vaani = get_vaani_integration()
        health_status = vaani.health_check()
        return {
            "vaani_healthy": health_status,
            "authenticated": vaani.access_token is not None,
            "base_url": vaani.base_url,
            "timestamp": datetime.now().isoformat()
        }
    except Exception as e:
        logger.error(f"Error checking Vaani health: {e}")
        return {
            "vaani_healthy": False,
            "authenticated": False,
            "error": str(e),
...
@app.get("/vaani/agent-config/{agent_id}")
async def get_vaani_agent_config(agent_id: str):
    """Get Vaani configuration for a specific agent."""
    try:
        from agent_bucket import get_agent_vaani_config
        config = get_agent_vaani_config(agent_id)
        if config:
            return {
                "agent_id": agent_id,
                "vaani_config": config,
            ...
            raise HTTPException(status_code=404, detail=f"No Vaani config found for agent {agent_id}")
    except Exception as e:
        logger.error(f"Error getting Vaani agent config: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def root():
    return {
        ...
        "endpoints": {
            ...
            "vaani": {
                "content_generate": "/vaani/content/generate - Generate content",
                "content_translate": "/vaani/content/translate - Translate content",
                "content_platform": "/vaani/content/platform - Publish content to platforms",
                "voice_generate": "/vaani/voice/generate - Generate voice"
            }
        },
        "documentation": "/docs"
    }
